...
encoder2 = torch.load('./models/vq_encoder_model.pkl')
...
def decode_fold(model, feature, tree, Boxes, Syms):
    def encode_node(node):
        if node.is_leaf():
            if config.finetune:
                b = node.box_noise
    ...
    def decode_node(feature, node, newBoxs, newSyms):
        if node.is_leaf():
            f = encode_node(node)
            ...
            decode_node(left_f, node.left, newBoxs, newSyms)
            decode_node(right_f, node.right, newBoxs, newSyms)
        elif node.is_sym():
            feature = model.vqlizationFeature(feature)
            ...
            decode_node(new_f, node.left, newBoxs, newSyms)

    decode_node(feature, tree.root, Boxes, Syms)
...
def inference(example):
    ...
    refineboxes = []
    syms = []
    decode_fold(model, enc_fold_nodes[0], example, refineboxes, syms)
    refineboxes = torch.cat(refineboxes, 0)
    if len(syms) == 0:
        syms = torch.zeros((1, 8))
    else:
        syms = torch.cat(syms, 0)
    refine_tree = Tree(refineboxes, example.ops, syms)
    return refine_tree


for batch_idx, batch in enumerate(test_iter):
    for example in batch:
        showGenshape(torch.cat(
            [example.leves[i].box for i in range(len(example.leves))], 0).data.cpu().numpy(),
                     save=False, savedir='./demo/'+str(batch_idx)+'_original.png')

    refine_tree = example
    for i in range(1):
        refine_tree = inference(refine_tree)
        showGenshape(torch.cat(
            [refine_tree.leves[i].box for i in range(len(refine_tree.leves))], 0).data.cpu().numpy(),
                     save=False, savedir='./demo/'+str(batch_idx)+'_original_recon.png')

    example.addNoise()
    showGenshape(torch.cat(
        [example.leves[i].box_noise for i in range(len(example.leves))], 0).data.cpu().numpy(),
                 save=False, savedir='./demo/'+str(batch_idx)+'_original_noise.png')
    ...
    for i in range(1):
        ...
        showGenshape(torch.cat(
            [refine_tree.leves[i].box for i in range(len(refine_tree.leves))], 0).data.cpu().numpy(),
                     save=False, savedir='./demo/'+str(batch_idx)+'_original_noise_recon.png')
